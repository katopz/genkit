...
use opentelemetry_sdk::trace::{
    BatchSpanProcessor, SdkTracerProvider, SimpleSpanProcessor, SpanData, SpanProcessor, TraceError,
};
...
pub fn flush_tracing() -> OTelSdkResult {
    if let Some(provider) = GENKIT_TRACER_PROVIDER.get() {
        ...
        OTelSdkResult::Err(opentelemetry_sdk::error::OTelSdkError::InternalFailure(
            "Failed to shutdown".to_owned(),
        ))
    }
}
