...
mod tests {
    ...
    fn test_parse_schema_valid() {
        let data = serde_json::json!({"foo": true});
        let schema = ProvidedSchema::FromType(schema_for::<TestStruct>());
        let parsed = parse_schema::<TestStruct>(data, schema).unwrap();
        assert_eq!(parsed, TestStruct { foo: true });
    }
    ...
    fn test_parse_schema_invalid_type() {
        ...
        let schema = ProvidedSchema::FromType(schema_for::<TestStruct>());
        let err = parse_schema::<TestStruct>(data, schema).unwrap_err();
    ...
    #[test]
    fn test_parse_schema_missing_field() {
        ...
        match err {
            Error::Validation(e) => {
                ...
                assert!(e.errors()[0].message.contains("is a required property"));
            }
            _ => panic!("Expected a validation error"),
        }
    }
    ...
    #[test]
    fn test_parse_nested_schema_invalid() {
        let data = serde_json::json!({"items": [{"foo": "not_a_bool"}]});
        let schema = ProvidedSchema::FromType(schema_for::<NestedStruct>());
        let err = parse_schema::<NestedStruct>(data, schema).unwrap_err();

        match err {
            Error::Validation(e) => {
                assert_eq!(e.errors().len(), 1);
                assert_eq!(e.errors()[0].path, "/items/0/foo");
            }
            _ => panic!("Expected a validation error"),
        }
    }
